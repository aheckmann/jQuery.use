...
(function($, p){
	...
	_use = function (pkg, callback, scope) {
		...
		function _load (deps, loader, doNotUseService) {
			...
			src = _resolve(dep.url.toLowerCase(), dep.isPkg, !usingService);
			...
			if ( _loadedPkgs.hasOwnProperty(src) ) {
				...
				while ( usingService && dep.isPkg ) {
					...
					dep = deps.length && deps[0].isPkg && deps.splice(0,1)[0];
				}
	...
	_getDeps = function (mod, isPackage) {
		...
		for (;i<len; i++) {
			...
			if (modules.hasOwnProperty(rq)) {							
				...
				if (rCSS.test( rq ))
					css.push({url: rq.replace("{THEME}", $.use.uiTheme), isPkg: pkg});
				else
	...
	_getURLsFromDOM = function () {	
		...
		;
		return $("script[src]").map( mapAttr('src') ).get().concat(
	 ...
	 *   pkg.overrides {bool} [optional] If true, this package will 
